...
// get question form field
const $addQuestionInput = $('#question');

// get submit button
const $addQuestionSubmitButton = $('#addQuestion');

// get ul id surveyList to add dynamic li
const $surveyList = $('#surveyList');


// submit button event handler
($addQuestionSubmitButton).on('click', function(event) {
   console.log('clicked!');
   event.preventDefault();

   // store value from user input
   let $questionValue = $addQuestionInput.val();
   console.log($questionValue);

   // create new list item with classes appended: form-group padout
   let $newSurveyListItem = $('<li class="form-group padout">');

   // append new item li to ol
   let $appendItemToList = $surveyList.append($newSurveyListItem);

   // inside new list item, add label
   let $newLabel = $('<label>');

   // append question value to the label
   let $appendQuestionToLabel = $newLabel.append($questionValue);
