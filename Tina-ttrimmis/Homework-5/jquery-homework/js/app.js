...
function addQuestionToList($list, question) {
    const $questionLi = $('<li>').html(question).addClass('form-group padout');
    $list.append($questionLi);
    $questionLi.html('<label>' + question + '</label><input type="text" class="form-control"><button class="removeQuestion">Remove question</button>');
}

$(document).ready(function () {
    const $surveyBuilder = $('#surveyList');
    const $addQuestion = $('#addQuestion');
    const $question = $('#question');
    const $removeQuestion = $('.removeQuestion');

    // Adds a question, input, & button to the survey
    $addQuestion.on('click', function (event) {
        ...
        if ($newQuestion === '') {
            ...
            addQuestionToList($surveyBuilder, $newQuestion);
            $question.val('');
        }
    });

    // Removes a question from the survery
...
// The below is refactored two event listeners into a single event listener for multiple events & it's also an example of event delegation I think?
