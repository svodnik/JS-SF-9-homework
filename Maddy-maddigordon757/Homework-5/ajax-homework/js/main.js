...
'use strict';
fetch('http://api.openweathermap.org/data/2.5/weather?zip=94123,us&appid=').then(function(response){
    return response.json();
       }).then(function(j) {
        //    console.log(j.main.temp);       
           const degF = (j.main.temp - 273.15) * 1.8 + 32;
           const degFInt = Math.floor(degF);
           console.log(degFInt);
        var para = document.querySelector('#temp');
        var node = document.createTextNode(degFInt);
       ...
       fetch('http://api.weatherunlocked.com/api/current/51.50,-0.12?app_id=').then(function(response){
        return response.json();
           ...
           console.log(j.temp_f);       
            var para = document.querySelector('#temp2');
            var node = document.createTextNode(j.temp_f);
            para.appendChild(node);
           });
