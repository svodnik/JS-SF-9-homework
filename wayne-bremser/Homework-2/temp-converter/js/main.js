/*
    ...
    2. Create a second function that accepts a temperature value in Celsius and converts it to Fahrenheit.
      ...
      function fahrenheitToCelsius (temp) {
            return outputtemp = ((temp - 32) / 1.8).toFixed(1);
      }

      function celsiusToFahrenheit (temp) {
            return outputtemp = ((temp * 1.8)  + 32).toFixed(1);
      }
    ...
    BONUS 1: Update your functions to use the value entered in the text box on the web page, and to perform calculations only when the appropriate button is clicked:
      ...
      BONUS 2: Display the result of each calculation in the browser window rather than in the console:
        ...
        */
        ...
        document.getElementById("fahrenheit_to_celsius").addEventListener("click", function runFahrenheitToCelsius (temp){
          let newtemp = document.querySelector("#temperature").value;
            console.log(fahrenheitToCelsius (newtemp));
            document.getElementById("result").textContent = fahrenheitToCelsius (newtemp) + "° Celsius ";
            document.querySelector("#temperature").value = '';

        });

        document.getElementById("celsius_to_fahrenheit").addEventListener("click", function runCelsiusToFahrenheit (temp){
          let newtemp = document.querySelector("#temperature").value;
            console.log(celsiusToFahrenheit (newtemp));
            document.getElementById("result").textContent = celsiusToFahrenheit (newtemp) + "° Farhenheit ";
            document.querySelector("#temperature").value = '';
        });
