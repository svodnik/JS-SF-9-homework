...
//**begin OpenWeatherData request**

const owdUrl = 'http://api.openweathermap.org/data/2.5/weather?q=san%20francisco&APPID=';
const owdKey = '{}';
const owdFullUrl = owdUrl + owdKey

function makeRequest(url) {
    console.log('Making request');
    fetch(owdFullUrl).then(function(response) {
        if (response.ok) {
        ...
        // console.log(data.main.temp)
          ...
          const degFInt = Math.floor(degF);
                  console.log(degFInt);
          const addTemp = document.querySelector('#temp');
          const newTemp = document.createElement('id');
          const tempValue = document.createTextNode(degFInt);
      newTemp.appendChild(tempValue);
      addTemp.appendChild(newTemp);
        });
    };

//**end OpenWeatherData request**

//**begin Weather Unlocked request**

const wuUrl = 'http://api.weatherunlocked.com/api/current/us.94117?app_id=188c66c9&app_key={}';
...
function secondRequest(url) {
    console.log('Making request');
      $.ajax({
        ...
        console.log(data.feelslike_f);
          const temp = (data.feelslike_f);
          const addTemp = document.querySelector('#temp2');
          const newTemp = document.createElement('id');
          const tempValue = document.createTextNode(temp);
      newTemp.appendChild(tempValue);
      addTemp.appendChild(newTemp);
        });
    };

//**end Weather Unlocked request**
